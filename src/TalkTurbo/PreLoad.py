...
from TalkTurbo.Messages import AssistantMessage, MessageRole, UserMessage


def _validate_pre_load_data(pre_load_data: list):
    if not isinstance(pre_load_data, list):
        raise TypeError(
            f"Pre-load data must be a list of dictionaries.  Data: {pre_load_data}"
        )

    for item in pre_load_data:
...
def get_pre_load_data(path: str) -> list:
